...
static const char* fragmentTemplateCommon =
    STRINGIFY(
        ...
        {
            vec2 coord = clamp(texCoord, u_textureSize / 2., vec2(1., 1.) - u_textureSize / 2.);
        ...
        vec4 sampleTextureClamp(sampler2D sampler, vec2 texCoord, vec2 offset, mat4 matrix)
        {
            ...
            coord = clamp(coord, u_textureSize / 2., vec2(1., 1.) - u_textureSize / 2.);
